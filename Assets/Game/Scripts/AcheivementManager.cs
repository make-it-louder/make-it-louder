...
public class AcheivementManager : MonoBehaviour
{
    ...
    public void AddConditions ()
    {
        ...
        conditions.Add(initialClearCount + playerMove2D.isClear >= 5);  // 10번 10번 클리어
        conditions.Add(initialClearCount + playerMove2D.isClear >= 10);  // 11번 102번 클리어
        conditions.Add(initialPlaytime + records["map1"].min_cleartime < 900 && playerMove2D.isClear == 1); // 12번 15분안에 클리어
        conditions.Add(initialJumpCount + records["map1"].count_minjump < 250 && playerMove2D.isClear == 1); // 13번 300번 점프 안에 클리어
        int cnt_avatars = 0; 
